...
public class Correlator
{
  ...
  // xHead and yHead come as separate arrayLists, whereas targets are all bunched together. 
  public int batchMatch(ArrayList<Long> timeHead, ArrayList<Float> xHead, ArrayList<Float> yHead, 
    ArrayList<Long> timeTargets, ArrayList<ArrayList<Float>> targetXs, ArrayList<ArrayList<Float>> targetYs)
  {
    duration = millis(); 
    // check all looks good sizewise (ignoaring time arrays here; should be fine as they are redundant)
    if (xHead.size()<windowSize && yHead.size()<windowSize && 
      targetXs.size()>0 && targetXs.size()!=numTargets && (targetXs.get(0)).size()<windowSize && // what is "targetXs.size()!=numTargets" doing?
      targetYs.size()>0 && targetYs.size()!=numTargets && (targetYs.get(0)).size()<windowSize)
      return -1;

    // prep for the set of correlations - we take the last windowSize items from the head arraylist and box it in an array
    xHeadArray     = convertTail(xHead);
    yHeadArray     = convertTail(yHead);
    timeHeadArray  = convertTailLong(timeHead);

    // this looks like a pretty slow way to do this calc... 
    DescriptiveStatistics statsX = new DescriptiveStatistics();
    DescriptiveStatistics statsY = new DescriptiveStatistics();
    for (int i=0; i<xHeadArray.length; i++)
    {
      statsX.addValue(xHeadArray[i]);
      statsY.addValue(xHeadArray[i]);
    }
    if (statsX.getStandardDeviation() < 2.0 || statsY.getStandardDeviation() < 2.0)
      return -4; // fail on low variance. 

    /*
    double[] indexes = new double[windowSize];
     for (int i=0;i<windowSize;i++)
     indexes[i] = i; 
     
     // detrend removes the regression slope
     xHeadArray = detrend(indexes, xHeadArray);
     yHeadArray = detrend(indexes, yHeadArray);
     */

    timeTargetsArray  = convertTailLong(timeTargets);
    // target zero...
    xTarget0Array  = convertTail(targetXs.get(0));
    yTarget0Array  = convertTail(targetYs.get(0));
    ...
    {
      // get the last windowSize items
      double[] xTargetArray = convertTail(targetXs.get(i));
      double[] yTargetArray = convertTail(targetYs.get(i));
