...
public class Correlator
{
  ...
  boolean lowSD;          // whether the results of the coorelation have a low sd
  ...
  // xyList[0][i]<-correlThreshold && xyList[1][i]>correlThreshold
  float getScore(int index)
  {
    if (corResults==null || index<0 || index>=corResults[0].length)
      return 0; 

    double score = ((-1.0 * corResults[0][index]) + corResults[1][index])/2; // mean score 
    
    if (score<0) return 0; // ignore subzero
  ...
  int getMaxScoreIndex()
    ...
    float maxScore = 0; 
    int maxScoreIndex = -1;
    for (int i=0;i<corResults[0].length;i++)
      {
      float s = getScore(i); 
      if (s>maxScore)
        {
                                 ...
                                 boolean[] enabled)
  {
    ...
    lowSD = false;                              // unknown sd
  
    // init the correl results
    ...
    // this we use to pull out the winner from a class va. Messy. Now set to nothing, set later if there is a winner this time round
    soloWinner = -1;
    ...
    // goal here is to run the correlations on any data sizes, up to windowSize in time
    /*
    ...
    if (headStartIndex<0)
      ...
      headStartIndex = 0; 
    long headDuration = timeHead.get(headSize-1) - timeHead.get(headStartIndex);
    ...
    if (targetsStartIndex<0) 
      ...
      targetsStartIndex = 0;
    long targetDuration = timeTargets.get(targetsSize-1) - timeTargets.get(targetsStartIndex);  
      
    // println("Head and target durations are " + headDuration + " and " + targetDuration + "ms long", timeHead.size(), timeTargets.size()); 
    ...
    if (xHeadArray==null || yHeadArray==null)
      return -3; // failed to get headStartIndex to headSize of data  
    ...
    if ((xsd + ysd) < 4) // look at the axes as a pair because SDs between axes may vary in the 1 second window
      {
      double range = statsX.getMax() - statsX.getMin();
      range += statsY.getMax() - statsY.getMin();
      if (range < 8)  // new criteria
        lowSD = true; // warn on low variance in head sample. This is just drift in the sensor. Fairly vaguely defined threshold.
      }
    ...
    for (int i=0; i<numTargets; i++)
       ...
       */
      if (xTargetArray!=null && yTargetArray!=null && xTargetArray.length>1 && enabled[i])
        {
        try {  
    ...
    if (headDuration>=windowSize) // HACK!! USE 1 SECOND TO SHOW HIGHLIGHT!
      {
      // pick a winner and return it
      int winner = checkCorrelations(corResults, targetDuration);
    ...
    return -1; // else return -1; no winner
  }
  ...
  int checkCorrelations(double[][] xyList, long duration) 
  {
    return checkCorrelationsCombi(xyList, duration);
  }
  ...
  int checkCorrelationsCombi(double[][] xyList, long duration) 
  {
    // see discussion at top of sketch, but we expect <<yaw neg to x>> and <<pitch pos to y>> 
    int numberCombWinners =  0; 

    ArrayList<Double> winnersStr = new ArrayList<Double>();
    ...
    for (int i=0; i<xyList[0].length; i++)
    {
      ...
      if (xyList[0][i]<-correlThreshold && xyList[1][i]>correlThreshold) // neg and pos
        {
        winners.add(i);
        winnersStr.add(xyList[1][i] - xyList[0][i]); 
        }
    ...
    soloWinner = -1;
    // calculate a leader for displaying feedback
    /*
    // this calculates a solowinner that is diffrent from the unique winner we actually return. Could be bad. 
    if (duration > MOMA_STARTUP_FEEDBACK)
      {
      double max = 0.0; 
      //print("wins: ");
      for (int i=0;i<winners.size();i++)
        {
        //print(winners.get(i) +", "+ winnersStr.get(i) +": ");
        if (winnersStr.get(i)>max)
          {
          max = winnersStr.get(i);
          soloWinner = winners.get(i); 
          }
        }
      }
      */
    ...
    } else if (winners.size()==1) // one winner where each axis beats the correl threshold
    {
      ...
      //println(all); 
      soloWinner = winners.get(0); // solowinner is exactly same as default calc.
