...
void dataStoreInit(int tNum)
{
  ...
  _targetXs    = new ArrayList<ArrayList<Float>>(tNum);
  _targetYs    = new ArrayList<ArrayList<Float>>(tNum);
...
void addTargetData(long now, float[] xs, float[] ys)
{
  ...
  // Remove old data
  if (_timeTargets.size()>2)
    {
    //println(_timeTargets.get(0), _timeTargets.get(_timeTargets.size()-2), correlator.windowSize);
    while (_timeTargets.get(0) - _timeTargets.get(_timeTargets.size()-2) > correlator.windowSize)
      {
...
void addHeadData(long now, float yaw, float pitch)
{
  ...
  if (_timeHead.size()>2)
    {
    //println(_timeHead.get(0), _timeHead.get(_timeHead.size()-2), correlator.windowSize);
    while (_timeHead.get(0) - _timeHead.get(_timeHead.size()-2) > correlator.windowSize)
      {
      _timeHead.remove(0);
      _xHead.remove(0);
      _yHead.remove(0);
