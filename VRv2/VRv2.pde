...
final int MODE_TEST = 4;                   // (only in MOMA)
// -
String SUBJECT = "_sub";
int subjectNum = 1;
final int BLOCKS = 3;                      // trial blocks (ignore the first?)
...
// -
int[] dwell_IN_count = new int [TARGET_NUM];          // count cursor touch
...
void calculate()
{
  ...
  if (mode == MODE_TEST) 
  {
    if (now <= test_timing+5000) 
    {
      cursor_pos = getCurrentCoords();
      float[] headRotation1 = new float[4];
      pvr.headTransform.getQuaternion(headRotation1, 0); 
      float[] yprVR1        = imuFilter.getYawPitchRoll(headRotation1);   // angles from the headtransform in the library
      if (yprVR1[0] >0)
      {
        yprVR1[0] = 180-yprVR1[0];
      } else if (yprVR1[0] <0)
      {
        yprVR1[0] = -yprVR1[0]-180;
      }
      test_time.append((float)now);
      test_x.append(cursor_pos.x);
      test_y.append(cursor_pos.y);
      test_VRx.append(yprVR1[1]);
      test_VRy.append(yprVR1[0]);
    } else 
    {
      mode = MODE_BREAK;
    }
  } else if (mode == MODE_TRIAL) 
  {
    ...
    } else 
    {
      ...
      if (inputMode == INPUT_DWELL) 
      {
        ...
        output_raw.print("CURSOR XY ,"+cursor_pos.x + "," + cursor_pos.y);
        //~~~~~~~~~~~~~~~~~~~
        if (dwellReady)    // Check if the cursor has moved away from the centre 
        {
  ...
  } else if (mode == MODE_CENTRE)      // Get the latest cursor coordinates
  {
    cursor_pos = getCurrentCoords();
...
void updateDwell(long now)  // Stores a possible first attempt at dwell
{
...
void checkCursorHover()    // Check if the cursor is over a target, and stores which (if any) 
{
  for (int i = 0; i<TARGET_NUM; i++) 
  {
...
void drawScene()      // Draws everything the user sees
{
...
void processIMuData(long now, long start, PGraphicsVR pvr)              // Correlates head and target data
{
  ...
  {
    if (now-start > MOMA_END_THRESHOLD)      // no match error 
    {
      winner = 99;
    }
...
void addHeadData(long now, float yaw, float pitch)
{
  ...
  _xHead.add(yaw);
  _yHead.add(pitch);
  if (_timeHead.size() > correlator.windowSize)        // Remove old data if we have more than windowSize samples    
  {
...
void dataStoreInit()                                    // Init. the data storage structures for head and target positions
{
  ...
  test_time = new FloatList();
  test_x = new FloatList();
  test_y = new FloatList();
  test_VRx = new FloatList();
  test_VRy = new FloatList();
...
String refreshTextMenu()
{
  ...
  if (mode == MODE_CHOOSE)
  {
    msg = "Input mode: " + MODE_STRINGS[inputMode] + "\nsubject "+subjectNum+"\n\nKeyboard buttons change\nRemote Button selects";
  } else if (mode == MODE_TEST) 
  {
    msg = " rotate head around till session ends";
...
void handleKeyEvent(KeyEvent event)                // Handle keyboard inputs
{
  int _keyCode = event.getKeyCode();
  if (event.getAction() == KeyEvent.RELEASE)
  {
    if (mode == MODE_CHOOSE)                       // What happens when the volume keys are pressed
    { 
    ...
    if (_keyCode == android.view.KeyEvent.KEYCODE_VOLUME_UP || _keyCode == android.view.KeyEvent.KEYCODE_SPACE)
    {
      ...
      { 
        ...
        output_raw = new PrintWriter( createWriter("//sdcard/Download/RAW_data_"+MODE_STRINGS[inputMode]+"_VRv2_"+SUBJECT+subjectNum+".csv"));
        output1 = new PrintWriter( createWriter("//sdcard/Download/response_data_"+MODE_STRINGS[inputMode]+"_VRv2_"+SUBJECT+subjectNum+".csv"));
...
void selectionTrigger()                  // The participant just selected a target
{
  if (mode == MODE_CENTRE)
  {
    if (inputMode == INPUT_MOMA)           // Prepare the trial
    {
      for (Target target : targets)            // Reset head/target data arrays, and each orbit to their original positions 
      {
        target.resetOrbit();
      }
