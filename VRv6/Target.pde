final float ANGLE_SPEED         = 3;   // 180 / (100/2);  // angle per second  (framerate / two eyes)
...
public class Target
{
  ...
  public void resetOrbit(int orbitResetConstant)
  {
  ...
  {    
    ...
    if (active)
    {
      ...
      target_fill = color(255, 0, 0, 155); // target is mid-alpha red 
    }
  ...
  // Move the orbit to reach ANGLE_SPEED_TARGET/second
  public void moveOrbit(long duration) 
  {
    float angularMovement = ((float)duration/1000.0) * ANGLE_SPEED_TARGET; // duration is in ms, but we need seconds. 
    
    angle += direction * angularMovement;
