...
final long HALFBREAK_TIME  = 6000;     // one minute in ms - we run a "long break" at the half-way point of the study. 
final long END_TIME        = 10000;     // ten seconds to end


// Study config
final int BLOCKS       = 6;                // Total trial blocks (ignore the first?) <- set as 20
      int TARGET_NUM   = 8;                // Total target numbers - in every corner : 8 targets, equals 8 trials per block - set in setup
boolean APPLY_DYNAMIC  = true;             // whether or not we adapt the targets position based on sensed feedback
...
final long  MOMA_STARTUP_FEEDBACK      = 6000;      // time to start feedback display - at the moment never
...
boolean useCenterDwell = true; // toggle if dwell on center passes to a new trial - the is immediate. TODO SET FALSE FOR REAL STUDY
...
void handleKeyEvent(KeyEvent event)
{
  ...
  //if (_keyCode == android.view.KeyEvent.KEYCODE_VOLUME_UP)   right = true;
