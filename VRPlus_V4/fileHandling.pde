...
{
  // VR_SPECIFIC - remove this lines
  // if (FOLDER == null) FOLDER = sketchPath() + "/data/";
...
void initLogFiles(String header)
{
  ...
  try {
    if (output_data ==null) {                    //Make data log file
      ...
      output_data.print("Targets: " + TARGET_NUM + ", Dynamic: " + APPLY_DYNAMIC + "\n");
      output_data.flush();
    }
  }
  catch (Exception e)
  {
    println("Error on creating "+DATAFILE+" log file with path " + header + DATAFILE + SUFFIX);
    fileError = true;
  }
  ...
  try {
    if (output_raw ==null) {                    //Make raw log file
      ...
      output_raw.print("Targets: " + TARGET_NUM + ", Dynamic: " + APPLY_DYNAMIC + "\n");
      output_raw.flush();
    }
  }
  catch (Exception e)
...
void writeStudyEnd()
{
  ...
  output_data.print("Targets: " + TARGET_NUM + ", Dynamic: " + APPLY_DYNAMIC + "\n");
  output_raw.print ("Targets: " + TARGET_NUM + ", Dynamic: " + APPLY_DYNAMIC + "\n");
  println("Targets: " + TARGET_NUM + ", Dynamic: " + APPLY_DYNAMIC);
