  ...
  switch (mode)
    ...
    case MODE_CHOOSE : 
      drawText("Input mode: " + MODE_STRINGS[inputMode] + "\nsubject "+subjectNum+
               "\nSpeed (F/S): " +MOMA_STARTUP +" & " + MOMA_DURATION + 
               "\nDynamic(D): " +APPLY_DYNAMIC+" & NumTargets(N): " + TARGET_NUM + 
               "\nHighlightForce(H): " + FORCE_HIGHLIGHT + 
...
void drawTargets(long now)
{
  ...
  drawTargetList(now, spinsCW, 0, (int)(-TARGET_SIZE*2));
  drawTargetList(now, spinsCCW, spinsCW.getSize(), (int)(+TARGET_SIZE*2)); 
}
