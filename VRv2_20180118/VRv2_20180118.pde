...
final int SCENE_DIST     = 1;          // scene distance from the viewer
PGraphics sceneSurf;
final int FRAMERATE      = 60;
final float TARGET_SIZE  = 40;         // target diam.

// INPUT PARAMETERS
final long MOMA_THRESHOLD = 1000;
final long MOMA_ACTIVATION_THRESHOLD = 500;
...
final int DWELL_THRESHOLD = 500;                           // we trigger dwell after 400ms -> 700ms
...
// -
final int BLOCKS = 2;                      // trial blocks (ignore the first?)
...
final float CURSOR_SIZE = SCENE_SIZE/30;    
...
void calculate()
{
  ...
  if (mode == MODE_TRIAL) {
    if (inputMode == INPUT_MOMA) {
      ...
      checkCursorHover();
      //  DATA POINT 
      //  current time + cursor X + cursor Y - non MOMA
      output1.print("CURSOR XY ,"+cursor_pos.x + "," + cursor_pos.y + "\n");
      output_raw.print("CURSOR XY ,"+cursor_pos.x + "," + cursor_pos.y);
        ...
        if (dwellReady) {
          ...
          if (now >= latestDwellMove + DWELL_THRESHOLD) {
            if (now >= dwellReady_time + DWELL_END_THRESHOLD) {            
              ...
              output1.print("\n###DWELL_COUNT_START,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              ...
              output1.print("\n");
              output_raw.print("\n");
              output1.print("###DWELL_COUNT_END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              output1.print("\n###DWELL_TIMES_START,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              output_raw.print("###DWELL_COUNT_END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              output_raw.print("\n###DWELL_TIMES_START,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              for (int i=0; i<TARGET_NUM; i++) {
                for (int j=0; j <10; j++) {
              ...
              output1.print("###DWELL_TIMES_END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              output_raw.print("###DWELL_TIMES_END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+ (over_target_num == trial_order.get(currentTrial))+","+(now-DWELL_TIMING)+"\n");
              selectionTrigger();
              dwell_avg = dwell_avg - DWELL_TIMING + now;
...
void drawScene()
{
  ...
  if (mode == MODE_TRIAL)
  {
    ...
    if (inputMode != INPUT_MOMA) drawCursor();
    //drawCursor();
  } else
  {
  ...
  // to indicate a correct/incorrect selection in the previous trial
...
void processIMuData(long now, long start, PGraphicsVR pvr)
{
  ...
  //println("HEAD : "  + " , " + yprVR[1] + " , " + yprVR[0] + "\n"); 
  //  DATA POINT
  //  head roatation data - MOMA
...
void dataStoreInit()
{
  ...
  //targets.add(new Target(0, tSpace, tSpace, random(360), 1, TARGET_SIZE));     // top left
  //targets.add(new Target(1, SCENE_SIZE/2, tSpace, random(360), -1, TARGET_SIZE));     // top middle
  //targets.add(new Target(0, SCENE_SIZE-tSpace, tSpace, random(360), 1, TARGET_SIZE));     // top right

  //// Middle (left to right)
  //targets.add(new Target(1, tSpace, SCENE_SIZE/2, random(360), -1, TARGET_SIZE));     // middle left
  //targets.add(new Target(1, SCENE_SIZE-tSpace, SCENE_SIZE/2, random(360), -1, TARGET_SIZE));     // middle right

  //// Bottom (left to right)
  //targets.add(new Target(0, tSpace, SCENE_SIZE-tSpace, random(360), 1, TARGET_SIZE));     // bottom left
  //targets.add(new Target(1, SCENE_SIZE/2, SCENE_SIZE-tSpace, random(360), -1, TARGET_SIZE));     // bottom middle
  //targets.add(new Target(0, SCENE_SIZE-tSpace, SCENE_SIZE-tSpace, random(360), 1, TARGET_SIZE));     // bottom right
}
...
void handleKeyEvent(KeyEvent event)
{
  ...
  if (event.getAction() == KeyEvent.RELEASE)
  {
    ...
    if (mode == MODE_CHOOSE)
    { 
      if (_keyCode == android.view.KeyEvent.KEYCODE_DPAD_RIGHT)    //KEYCODE_VOLUME_DOWN    KEYCODE_DPAD_RIGHT
      {
        inputMode++; 
        if (inputMode >= MODE_STRINGS.length) inputMode = 0;
      } else if (_keyCode == android.view.KeyEvent.KEYCODE_DPAD_LEFT)    //KEYCODE_VOLUME_UP
      {
        inputMode--; 
    ...
    if (_keyCode == android.view.KeyEvent.KEYCODE_SPACE)      //  KEYCODE_SPACE
    {
      ...
      } else if (mode == MODE_CHOOSE)
      {
        ...
        //  DATA POINT
        // set filename
        output_raw = new PrintWriter( createWriter("//sdcard/Download/RAW_data_"+MODE_STRINGS[inputMode]+"_VRv2.csv"));
        output1 = new PrintWriter( createWriter("//sdcard/Download/response_data_"+MODE_STRINGS[inputMode]+"_VRv2.csv"));
...
void selectionTrigger()
{
  ...
  } else if (mode == MODE_TRIAL)
  {
    ...
    if (inputMode == INPUT_KEY) {
      //KEYBOARD_TIMING = millis();
      keyboard_avg = keyboard_avg + millis() - KEYBOARD_TIMING;
      output1.print("###END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+","+(over_target_num == trial_order.get(currentTrial))+","+(millis() - KEYBOARD_TIMING)+"\n");
      output_raw.print("###END,"+currentBlock+","+currentTrial+","+trial_order.get(currentTrial)+","+(over_target_num == trial_order.get(currentTrial))+","+(millis() - KEYBOARD_TIMING)+"\n");
    }
  }
}
